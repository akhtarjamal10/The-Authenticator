...
export const AdminAuth: React.FC<AuthProps> = ({ onLogin }) => {
  const [activeTab, setActiveTab] = useState<UserRole>(UserRole.ADMIN);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  
  // 1. Remove hardcoded credentials from initial state
  ...
  const handleStandardLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Real Logic: Call the API
      if (activeTab === UserRole.ADMIN) {
        user = await api.loginAdmin(formData.id, formData.password);
      }
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-slate-100 flex items-center justify-center py-12 px-4">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg border-t-4 border-gov-blue">
        ...
        {/* Role Tab (Visual only since it's Admin-only) */}
        <div className="flex border-b border-gray-200 mb-6">
          <button className="flex-1 py-3 text-sm font-medium border-b-2 flex justify-center gap-2 border-gov-blue text-gov-blue">
             Administrator
          </button>
        </div>
        ...
        <form className="space-y-6" onSubmit={handleStandardLogin}>
          <input 
            ...
            required 
            className="w-full px-3 py-3 border border-gray-300 rounded" 
            placeholder="Admin Email (e.g. jk@gov.in)" 
            value={formData.id} 
            onChange={e => setFormData({...formData, id: e.target.value})} 
          />
          <input 
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="w-full py-3 bg-gov-blue text-white rounded shadow-lg hover:bg-blue-900 font-bold flex justify-center"
          >
